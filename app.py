    ...
    if req.api_key and req.model_id and req.model_id not in {"gemini-2.5-flash", "cohere"}:
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=45) as client:
                selected_model = model_names.get(req.model_id, "gpt-4o-mini")
                if req.model_id != "anthropic":
                    models_url = bases.get(req.model_id, "").replace("/chat/completions", "/models")
                    if models_url:
                        model_response = await client.get(models_url, headers=headers)
                        if model_response.status_code == 200:
                            available = [item.get("id", "") for item in model_response.json().get("data", [])]
                            usable = [x for x in available if not any(word in x.lower() for word in ("whisper", "guard", "embed", "moderation", "tts", "audio"))]
                            preferred = [selected_model, "llama-3.1-8b-instant", "llama-3.3-70b-versatile", "openai/gpt-oss-20b", "mistral-small-latest"]
                            selected_model = next((x for x in preferred if x in usable), usable[0] if usable else selected_model)
                payload = {"model": selected_model, "messages":[{"role":"user","content":req.prompt}], "temperature":0.3}
            ...
            return {"status":"success","inferred_mode":mode,"summary":answer,"steps":[{"phase":"thinking","agent":"Thanos Router","specialty":"Provider selection","status":f"Selected {req.model_id} and discovered an available model.","output":f"Using model: {selected_model}"},{"phase":"executing","agent":selected_model,"specialty":"Live provider response","status":"Completed real API request.","output":"Response returned by the selected provider."}]}
        except httpx.HTTPError as exc:
